    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      <View style={{ backgroundColor: colors.card, paddingVertical: 12, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <View style={{ alignItems: 'center' }}>
          ...
          <Text style={{ color: colors.muted, marginTop: 2 }}>{state.language==='de'?'Plan vs. Ist, Pace, Trend & BMI':(state.language==='pl'?'Plan vs. Rzecz., tempo, trend i BMI':'Plan vs. actual, pace, trend & BMI')}</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.card }]}> 
          ...
          {info ? (
            <Text style={{ color: colors.muted, marginTop: 6 }}>
              Diese Seite wird im nächsten Schritt mit: Plan vs. Ist, Pace (7d), ETA/Trend, BMI sowie Analyse (Trend/Plateau/Korrelationen) und lokalen KI-Kurztipps umgesetzt.
            </Text>
          ) : null}
        </View>
      </ScrollView>
